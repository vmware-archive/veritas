...
func AssetServer(dev bool) http.HandlerFunc {
	if dev {
		...
		return func(w http.ResponseWriter, r *http.Request) {
			...
			f, err := os.Open(filepath.Join("./chug_commands/assets", fname))
			if err != nil {
...
func regenerateAssets() {
	dir, err := ioutil.ReadDir("./chug_commands/assets")
	if err != nil {
		panic(err)
	}

	out, err := os.Create("./chug_commands/assets.go")
	if err != nil {
		panic(err)
	}

	out.WriteString(`package chug_commands

var assets = map[string]string{
`)

	for _, info := range dir {
		f, err := os.Open(filepath.Join("./chug_commands/assets", info.Name()))
		if err != nil {
			panic(err)
		}
	...
	err = exec.Command("go", "fmt", "./chug_commands/assets.go").Run()
	if err != nil {
		panic(err)
	}
}
